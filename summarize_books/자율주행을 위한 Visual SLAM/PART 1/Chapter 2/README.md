...
$$ 
  \begin{cases}
    ...
    z_{k,j} = h(y_j, x_k, v_{k,j}),  & \quad (k,j) \in \mathcal{O}
  \end{cases}
$$

여기서 $\mathcal{O}$는 어떤 랜드마크가 어느 순간에 관찰되었는지 기록하는 집합이다(일반적으로 모든 랜드마크가 매 순간 표시되는 것이 아니다. 어떤 순간에 그중 아주 작은 부분만 볼 수 있는 경우도 있다). 이 두 방정식은 가장 기본적인 SLAM 문제를 설명한다. 노이즈를 포함한 모션 명령값 u와 센서의 판독값 z를 알고 있을 때 위치 추정 문제($x$ 추정)와 매핑 문제($y$ 추정)를 해결하는 방법은 무엇일까? 이 시점에서 우리는 SLAM 문제를 상태 추정 문제로 모델링한다. 노이즈가 있는 측정 데이터를 통해 내부의 숨겨진 상태 변수를 추정하는 방법은 무엇일까?

상태 추정 문제의 솔루션은 두 방정식의 특정 형식과 잡음이 따르는 분포와 관련이 있다. 모션 모델방정식과 관찰 모델방정식이 선형인지 비선형인지 노이즈가 가우스 분포를 따르는지에 따라 선형/비선형과 가우스/비 가우스로 분류된다. 그중 선형 가우스(Linear Gaussian, LG) 시스템이 가장 간단하며 칼만 필터(Kalman Filter, KF)를 통해 편향 없는 최적 추정값을 얻을 수 있다. 복잡한 비선형 비가우시안 시스템(Non-Linear Non-Gaussian, NLNG)에서는 확장 칼만 필터(Extended Kalman Filter, EKF)와 비선형 최적화 방법을 사용하여 해결하였다. 오늘날 주류 Visual SLAM은 그래프 최적화(Graph Optimization)로 대표되는 최적화 기술을 사용하여 상태를 추정한다. 

## 2.4 실습 : 프로그래밍의 기초

### 2.4.1 Linux 운영 체제를 설치

이 책의 프로그램은 주로 Linux의 C++ 프로그램이고 실험 중에 많은 라이브러리를 사용한다.

### 2.4.2 Hello SLAM

많은 컴퓨터 책과 마찬가지고 Hello SLAM 프로그램을 작성으로 시작한다. 

Linux에서 프로그램은 실행 권한이 있는 파일이다. 스크립트 또는 바이너리일 수 있지만 접미사 이름을 한정하지는 않는다(Windows에서 .exe 파일로 지정해야 하는 경우는 제외). 일반저긍로 사용하는 cd, ls 및 기타 명령은 /bin 디렉터리에 있는 실행 파일이다. 

텍스트 편집기 gedit(또는 Vim)을 사용하여 코드를 입력하고 나열된 경로 아래에 저장할 수 있다.

### 2.4.3 cmake를 사용하기

이론적으로 모든 C++ 프로그램은 g++ 컴파일할 수 있다. 그러나 프로그램이 점점 더 커질 때 프로젝트에 많은 폴더와 소스 파일이 있을 수 있으며 컴파일 명령을 입력하는 것이 점점 더 길어질 수 있다. g++ 명령만으로는 많은 수의 컴파일 명령을 입력해야 하며 전체 컴파일 프로세스가 비정상적으로 번거로울 수 있따. 따라서 C++ 프로젝트의 경우 일부 엔지니어링 관리 도구를 사용하는 것이 더 효율적이다. 

cmake 프로젝트에서는 cmake 명령을 사용하여 makefile 파일을 생성한 다음 make 명령을 사용하여 makefile 파일의 내용을 기반으로 전체 프로젝트를 컴파일한다. 위의 helloSLAM.cpp 예로 들자면, 이번에는 g++를 직접 사용하는 대신 cmake를 사용하여 프로젝트를 만든 다음 컴파일한다.

CMakeLists.txt 파일은 cmake에게 이 디렉터리의 파일에 대해 수행할 작업을 알려준다. CMakeList.txt 파일의 내용은 cmake 구문을 따라야 한다.

이제 현재 디렉터리에서 cmake를 호출하여 프로젝트를 cmake 컴파일한다.

cmake는 일부 컴파일 정보를 출력한 다음 현재 디렉터리에서 중간 파일을 생성하는데 그중 가장 중요한 것이 makefile이다. Makefile은 자동으로 생성되므로 수정할 필요가 없다. 이제 make 명령을 사용하여 프로젝트를 컴파일한다.

### 2.4.4 라이브러리 사용

C++ 프로젝트에서 모든 코드가 실행 파일로 컴파일되는 것은 아니다. main 함수가 있는 파일만 실행 프로그램을 생성한다. 다른 코드는 다른 프로그램에서 호출할 수 있도록 한 가지로 패키징 하려고 한다. 이것을 라이브러리라고 한다.

Linux에서 라이브러리 파일은 정적 라이브러리와 공유 라이브러리로 나뉜다.(동적 라이브러리라고 하지는 않는다.) 정적 라이브러리는 .a 접미사 이름으로, 공유 라이브러리는 .so로 끝난다. 모든 라이브러리는 패키징된 함수의 컬렉션이며 정적 라이브러리는 호출될 때마다 복사본을 생성하는 반면 공유 라이브러리에는 복사본이 하나만 있으므로 공간이 적다는 것이 차이점이다. 그리고 정적 라이브러리 대신 공유 라이브러리를 생성하려면 다음과 같은 명령문을 사용한다.

1 add_library( hello_shared SHARED libHelloSLAM.cpp )

이 시점에서 얻은 파일은 libhello_shared.so이다.

라이브러리 파일은 컴파일된 이진 함수가 포함된 압축 패키지이다. .a또는 .so 라이브러리 파일만 있는 경우 함수가 무엇인지 호출의 형식이 무엇인지는 알 수 없다. 다른 사람(또는 자신)이 라이브러리를 사용하려면 이러한 라이브러리에 무엇이 있는지 설명하는 헤더 파일을 제공해야 한다. 따라서 라이브러리의 사용자는 헤더 및 라이브러리 파일만 있으며 이 라이브러리를 호출할 수 있다.

### 2.4.5 IDE를 사용

IDE(통합 개발 환경, Integrated Development Environment)는 개발자에게 점프, 컴파일, 브레이크포인트 디버깅 등과 같은 많은 편리한 기능을 제공한다. 우리가 사용할 IDE는 KDevelop으로, KDevelop은 우분투 시스템의 소프트웨어 저장소에 위치한 무료 소프트웨어로, 다음과 같은 장점이 있다.

1. cmake 프로젝트를 지원한다.
2. C++에 대한 지원이 더 좋다. 하이라이팅, 점핑, 컴파일 및 기타 기능이 있으며 코드를 자동으로 타이포그래피 할 수 있다.
3. 개별 파일 및 디렉터리 트리를 쉽게 볼 수 있다.
4. 원클릭 컴파일, 브레이크포인트 디버깅 및 기타 기능이 있다.

### 2.4.5.1 KDevelop의 사용

KDevelop은 cmake 프로젝트를 기본으로 지원한다. KDevelop의 브레이크포인트 디버깅 기능을 사용하려면 다음을 수행해야 한다.

1. 최적화 옵션을 사용하지 않고 CMakeLists.txt에서 프로젝트를 Debug 컴파일 모드로 전환한다(기본적으로 사용되지 않음).
2. KDevelop에게 실행할 프로그램을 알려준다. 매개변수가 있는 경우 매개변수 및 작업 디렉토리를 구성한다.
3. 브레이크포인트 디버깅 인터페이스를 입력하면 스텝마다 중간 변수의 값을 볼 수 있다.
