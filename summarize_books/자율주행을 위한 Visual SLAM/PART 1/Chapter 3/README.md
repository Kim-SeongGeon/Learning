...
회전을 설명하기 위해 회전 행렬을 사용하면 6자유도의 3D 강체 모션을 설명하는 4 x 4 변환 행렬로 충분할까? 행렬 표현은 다음과 같은 두 가지 단점이 있다:

1. SO(3)의 회전 행렬은 9개의 요소를 가지지만 3D 회전은 한 번에 3개의 자유도만 회전한다. 따라서 이 표현은 중복되는 요소가 있는 것이다. 마찬가지로 4 x 4 변환 행렬은 6 자유도의 변환을 16개의 요소로 표현한다.
2. 회전 행렬 자체에는 제약 조건이 있다: 직교 행렬이어야 하며 행렬식은 1이어야 한다. 이것은 변환 행렬도 마찬가지이다. 이러한 제약 조건으로 인해 회전 또는 변환 행렬을 추정하거나 최적화하려는 경우 더 어렵게 만든다.

실제로 회전축과 회전 각도로 모든 회전을 묘사할 수 있다. 따라서 회전축과 똑같은 방향으로 벡터를 사용하고 길이는 회전하는 각도와 같게 할 수 있다. 이 벡터를 회전 벡터(또는 angle-axis, axis-angle)라고 하며 회전을 설명하기 위해 단지 3차원 벡터만 필요하다. 마찬가지로 변환 행렬의 경우 회전 벡터와 병진 벡터를 사용하여 변환을 한 번 표현한다. 이 시점에서 변수 차원은 정확히 6차원이다.

### 3.3.2 오일러 각도

오일러 각도는 회전을 표현하는 매우 직관적인 방법으로 서로 다른 3개의 축을 이용해 회전을 분해하여 표현하는 방법이다. 하나의 단일 축을 중심으로 회전하는 과정은 쉽게 이해할 수 있기 때문에 직관적이라고 말할 수 있다. 그러나 분해하는 방법에는 여러 가지가 있으므로 오일러 각도에는 혼동하기 쉬운 다양한 정의 방법이 있다. 예를 들어, $X$축을 중심으로 한 다음 $Y$축을 중심으로 회전한 후 $Z$축을 중심으로 회전하면 $XYZ$축의 회전이 발생한다. 

이 회전축 정의의 불확실성은 많은 실질적인 어려움을 야기하고 있지만 다행히도 오일러 각도는 일반적으로 특정 분야에서 균일한 정의 방법을 가지고 있다. 오일러 각도에서 일반적으로 사용되는 것 중 하나는 요-피치-롤(yaw-pitch-roll)의 세 가지 각도로 회전을 설명하는 것이다. 이는 $ZYX$축의 회전과 동일하므로 $ZYX$를 예로 들어 보면, 강체의 전면(우리를 향한 방향)이 $X$축이고 오른쪽을 $Y$축과 위쪽을 $Z$축으로 가정한다. 따라서 $ZYX$코너는 임의의 회전을 다음 세 축으로 분해하는 것과 같다:

1. 물체의 $Z$축을 중심으로 회전하여 요 각도 yaw를 얻는다.
2. 회전 후 $Y$축을 중심으로 회전하여 피치 각도 pitch를 얻는다.
3. 회전 후 $X$축을 중심으로 회전하여 롤 각도 roll를 얻는다.

이 시점에서 $[r, p, y]^T$와 같은 3차원 벡터는 임의 회전을 설명한다. 다른 오일러 각도도 이러한 방식으로 회전을 3축으로 분해하여 3차원 벡터를 생성하지만 선택한 축과 순서는 다르다. 위에서 언급했듯이 rpy 각도의 회전 순서는 $ZYX$이다.

오일러 각도의 가장 큰 단점은 유명한 짐벌락(Gimbal Lock) 문제인데 ypr 회전에서 피치 각도가 $90^\circ$인 경우 첫 번째 회전과 세 번째 회전은 동일한 축을 사용하게 되어 시스템의 자유도 하나를 잃게 된다(3회전에서 2회전으로 변경). 이를 특이점(singularity) 문제라고 하며 다른 형태의 오일러 각에도 존재한다. 이러한 점 때문에 오일러 각도는 보간 및 반복에 적합하지 않으며, 또한 SLAM 프로그램에서 직접 오일러 각도를 사용하여 포즈를 표현하는 경우는 거의 없으며 필터 또는 최적화에서 오일러 각도 표현 회전을 거의 사용하지 않는다(이유는 당연히 특이점 현상으로 인해). 그러나 알고리즘에 오류가 있는지 확인하려는 경우 오일러 각도로 변환하는 것이 우리에게 더 직관적이므로 결과가 올바른지 빠르게 확인할 수 있다. 2D 모션(예: 청소 로봇, 자율주행 차량)이 주로 나타나는 경우, 회전을 세 개의 오일러 각도로 분할 한 다음 그중 하나(예: 요 각도)를 방향 정보 출력으로 내보낼 수도 있다. 왜냐하면 모션이 2D로 제한되므로 나머지(예: 피치 및 롤 각도)는 모션이 없다고 가정할 수 있으므로 그 값을 0으로 볼 수 있기 때문이다.
