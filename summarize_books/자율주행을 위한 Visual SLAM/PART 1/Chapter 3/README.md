...
$\text{SO}(n)$는 특수 직교 군(Special Orthoogonal Group)을 뜻한다. 이 집합은 $n$차원 공간의 회전 행렬로 구성되며 특히 $\text{SO}(3)$는 3차원 공간의 회전을 의미한다. 행렬을 회전하면 베이시스 벡터에서 시작하는 대신 두 좌표계 간의 회전 변환에 대해 직접 이야기할 수 있다.
