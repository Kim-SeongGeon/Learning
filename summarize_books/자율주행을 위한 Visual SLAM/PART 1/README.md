...
두 번째 관찰에 해당하는 모델방정식도 있다. 관찰 모델은 순무가 $x_k$의 위체에서 특정 랜드마크 $y_j$를 볼 때 관찰 데이터 $z_{k,j}$가 생성된다는 것을 나타낸다. 모션 모델에서 추상 함수 $f$를 사용하여 표현했다면 관찰 모델에서는 추상 함수 $h$를 사용하여 이 관계를 설명한다.
